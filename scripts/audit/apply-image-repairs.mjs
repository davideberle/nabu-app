...
// Rule R1-reviewed-icon: only assignments explicitly retained in the reviewed
// evidence file are eligible. Image dimensions are candidate-generation evidence,
// never an automatic repair rule. The asset remains for traceability.
...
const reviewedRecord = JSON.parse(readFileSync(OUT, "utf8"));
if (reviewedRecord.reviewStatus !== "visually-reviewed") throw new Error(`repair evidence ${OUT} is not marked visually-reviewed`);
const reviewedRecipeIds = new Set(reviewedRecord.repairs.map((r) => r.recipeId));
const repairs = selectRepairs(audit, reviewedRecipeIds);
const applied = [];
...
const record = { appliedAt: new Date().toISOString(), dryRun: !!args["dry-run"], reviewStatus: "visually-reviewed", rule: "R1-reviewed-icon explicit allowlist -> image:null", count: applied.length, repairs: applied };
