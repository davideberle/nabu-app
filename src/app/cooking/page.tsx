...
import type { CookingSession, SessionIngredient, CoachCards, RelatedRecipe } from "@/lib/cooking";
    ...
    <>
      ...
      <section className="rounded-xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-5">
        <p className="text-xs tracking-widest uppercase text-stone-400 dark:text-stone-500 mb-1">
          ...
          {session.anchor.provenance.author && (
            <span> &middot; {session.anchor.provenance.author}</span>
          )}
        </p>
      ...
      <section className="rounded-xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-5">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-xs tracking-widest uppercase text-stone-400 dark:text-stone-500">
            Ingredients
          </h3>
          {hasSessionIngredients && (
...
// ---------------------------------------------------------------------------
// Tonight: related recipes + serve with
// ---------------------------------------------------------------------------
...
function TonightSection({
  ...
  if (relatedRecipes.length === 0 && serveWith.length === 0) return null;
    ...
    <section className="rounded-xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 p-5">
      ...
      {relatedRecipes.length > 0 && (
        <ul className="space-y-2 mb-3">
          {relatedRecipes.map((r) => (
            <li key={r.recipeId} className="flex items-center gap-2 text-sm">
              <span className="shrink-0 text-[10px] px-2 py-0.5 rounded-full bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400">
                {r.kind}
              </span>
              <a
      ...
      {serveWith.length > 0 && (
        <div className="text-sm text-stone-500 dark:text-stone-400">
          <span className="font-medium text-stone-600 dark:text-stone-300">Serve with: </span>
          {serveWith.join(", ")}
        </div>
      )}
    </section>
...
function IngredientRow({ ing }: { ing: SessionIngredient }) {
  return (
    <li className="text-sm text-stone-600 dark:text-stone-400 flex justify-between">
      <span>{ing.item}</span>
