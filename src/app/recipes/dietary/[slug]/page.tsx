...
export async function generateStaticParams() {
  ...
  return options.map((d) => ({ slug: d.slug }));
}

export default async function DietaryPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const options = await getDietaryOptions();
  const option = options.find(d => d.slug === slug);

  if (!option) {
    notFound();
  }

  const recipes = await getRecipesByDietary(slug);
