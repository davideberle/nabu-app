...
- 2026-06-03: Live cooking polish prepared for deployment. `/cooking` now keeps curry/simmer recipes out of the salad flow so Thai curry no longer says to make dressing first, filters stale/drink/backstory/filler session notes, and renders useful notes as a quieter "Tonight's notes" block near the top. `npx tsc --noEmit`, `npm run build`, and independent Claude verification passed.
